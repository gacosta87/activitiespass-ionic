...
FROM 
  ...
  INNER JOIN `categories_tours` `CategoriesTours` ON `Categories`.`id` = (
    `CategoriesTours`.`category_id`
  ) 
WHERE 
  `CategoriesTours`.`tour_id` in (1)
...
WHERE 
  `ToursProvidedOptions`.`tour_id` in (1)
...
WHERE 
  (
    ...
    AND `TourSchedules`.`tour_id` in (1)
  )
