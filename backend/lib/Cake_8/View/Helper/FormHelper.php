 ...
 */
	public function input($fieldName, $options = array()) {
		...
		//CAMPO FECHA COLOCARLE EL CALENDAR
		$date_  = 0;
		$script = "";
		if ($options['type'] === 'date') {
			...
			/*$script = " <script> $('#".$options['id']."').datepicker();</script><span class='input-group-addon add-on'><span class='glyphicon glyphicon-calendar'></span></span>";*/
		    $date_  = 1;
		}
		if ($options['type'] === 'date2') {
			$options['type'] = 'text';
			$options['value'] = !isset($options['value'])?date('Y-m-d'):$options['value'];
			$options['readonly'] = true;
			$script = " <script> $('#".$options['id']."').datepicker();</script><span class='input-group-addon add-on'><span class='glyphicon glyphicon-calendar'></span></span>";
		    $date_  = 1;
		}
		if ($options['type'] === 'date3') {
			$options['type'] = 'text';
			//$options['value'] = !isset($options['value'])?date('Y-m-d'):$options['value'];
			$options['readonly'] = true;
			$script = " <script> $('#".$options['id']."').datepicker();</script><span class='input-group-addon add-on'><span class='glyphicon glyphicon-calendar'></span></span>";
		...
		if ($options['type'] === 'number') {
			$options['onKeyPress'] = 'return solonumeros_con_punto(event);';
			$options['type'] = "text";
		}
		if ($options['type'] === 'numbersinpunto') {
		...
		if ($options['type'] === 'select') {
			$options['style'] = "height: 44px !important;";
		}
		...
		if ($options['type'] === 'time') { 
			...
			$script = " <script> 
            var options = { now: \"".date('H').":00\", //hh:mm 24 hour format only, defaults to current time
							...
							secondsInterval: 0, //Change interval for seconds, defaults to 1
							minutesInterval: 30, //Change interval for minutes, defaults to 1
							beforeShow: null, //A function to be called before the Wickedpicker is shown
							show: null, //A function to be called when the Wickedpicker is shown
							clearable: false, //Make the picker's input clearable (has clickable \"x\")
						   }; 
			$('#".$options['id']."').wickedpicker(options);</script>";
		    //$date_  = 1;
		}
		//CAMPO DE HORAS
		if ($options['type'] === 'time2') { 
			$options['type'] = 'text';
			//$options['class'] = 'timepicker';
			//$options['value'] = date('Y-m-d');
			//$options['readonly'] = true;
			...
			$script = " <script> 
            var options = { now: \"".$options['value']."\", //hh:mm 24 hour format only, defaults to current time
							twentyFour: true, //Display 24 hour format, defaults to false
							upArrow: 'wickedpicker__controls__control-up', //The up arrow class selector to use, for custom CSS
							downArrow: 'wickedpicker__controls__control-down', //The down arrow class selector to use, for custom CSS
							close: 'wickedpicker__close', //The close class selector to use, for custom CSS
       ...
       // pr($out['input']);
		...
		/*if($date_==1){
            return "<div class='col-md-11'><div class='input-group date datepickershort'>".$output.$script."</div></div>";
		}else{
			if($require!=""){
				return "<div class='col-md-11'>".$output."</div> <div class='col-md-1' style='color:red;'>".$require."</div> ".$script;
			}else{
                return "<div class='col-md-11'>".$output."</div>".$script;
			}
		}*/
		if($date_==1){
            return "<div class='input-group date datepickershort'>".$output.$script."</div>";
		}else{
