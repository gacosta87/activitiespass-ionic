...
@Component({
  selector: 'app-perfilchat',
  templateUrl: 'perfilchat.html',
  styleUrls: ['perfilchat.scss'],
  providers:[Variablesglobales, Home, Camera, File, Crop, MediaCapture, AndroidPermissions, VideoEditor, Geolocation]
})
export class Perfilchat implements  OnInit{
  ...
  ionViewDidEnter(){ 
      ...
      const loader = this.loadingCtrl.create({
              //qmessage: "Un momento, por favor..."
                                 ...
                                 this.providerhome.chatlist(this.usuarioid).subscribe((response) => {  
                                      this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
  ...
  newchat(){
    this.navController.navigateForward("/perfilchattema");  
  }


  linkperfil(var1){
    this.navController.navigateForward("/perfilchatnew/0/"+var1);   
  }



  edit(var1){
    this.navController.navigateForward("/perfilchatedit/"+var1);   
  }
                                        ...
                                        handler: data => {
                                            const loader = this.loadingCtrl.create({
                                              //qmessage: "Un momento, por favor..."
                                                  ...
                                                  this.providerhome.chatdel(var1, this.usuarioid).subscribe((response) => {  
                                                        this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                                                      if(response['code']==200){
                                                                                this.datos  = response['datos'];
                                                                      }else if (response['code']==201){
