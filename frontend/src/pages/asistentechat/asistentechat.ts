...
@Component({
  selector: 'app-asistentechat',
  templateUrl: 'asistentechat.html',
  styleUrls: ['asistentechat.scss'],
...
export class Asistentechat implements  OnInit{
    ...
    public mensajeaux = "";
    public inicio = 1;
    public datos_mycar: any;
    public p_idioma = "en";
    public asistente_usuario_id = "";
    public asistente_mymotor_id = "";
  ...
  ionViewDidEnter(){ 
      ...
      this.asistente_usuario_id = this.rutaActiva.snapshot.paramMap.get('asistente_usuario_id');
      this.asistente_mymotor_id = this.rutaActiva.snapshot.paramMap.get('asistente_mymotor_id');
      ...
      const loader = this.loadingCtrl.create({
              //qmessage: "Un momento, por favor..."
                      ...
                      this.providerhome.asistentechatlist(this.asistente_usuario_id, this.asistente_mymotor_id, this.p_idioma).subscribe((response) => {  
                            this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                          if(response['code']==200){
                                                    ...
                                                    this.datos_mycar = response['datos_mycar'];
                                                    this.inicio      = 1;
...
cargartecto(){
  ...
  let texto = this.data[this.data.length-1]['mensaje'];
...
copiar(texto){
     ...
     if(texto=="ultimo001"){
        texto = this.data[(this.data.length-1)]['mensaje'];
     }
        ...
        if(this.myForm.value.texto!="" && this.mensajeaux!=this.myForm.value.texto && this.limite>=this.myForm.value.texto.length){
            ...
            const loader = this.loadingCtrl.create({
              ...
              cssClass: 'loading-back'
                                    ...
                                    load.present();
                                    this.data.push({'role':'user', 'mensaje':this.mensajeaux});
                                    let elem: HTMLElement = document.getElementById('ultimo');
                                    elem.innerHTML="";
                                    this.inicio = 3;
                                    console.log('enviar: '+this.chatlist_id);
                                    this.scrollToBottomOnInit(); 
                                    this.providerhome.asistentechatadd( this.asistente_usuario_id, this.asistente_mymotor_id, this.mensajeaux).subscribe((response) => {  
                                                this.loadingCtrl.getTop().then(loader => {
                                                      if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                                      if(response['code']==200){
                                                                this.data.push({'role':'assistant', 'mensaje':response['response']['content']});
                                                                this.puntos      = response['puntos'];
                                                                this.limite      = response['limite'];
                                                                this.contar_chat = response['contar_chat'];
                                                                this.limite_chat = response['limite_chat'];
