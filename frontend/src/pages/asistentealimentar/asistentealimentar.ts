...
@Component({
  selector: 'app-asistentealimentar',
  templateUrl: 'asistentealimentar.html',
  styleUrls: ['asistentealimentar.scss'],
  providers:[Usuario, Home, Variablesglobales, Camera, File, Crop, GooglePlus, Facebook, Instagram, Geolocation]
})

export class Asistentealimentar implements  OnInit{
  ...
  enviarformulario() {
    const loader = this.loadingCtrl.create({
      ...
      //message: "Un momento, por favor..."
              ...
              this.provider.miperfilupdate_asistente_upload(this.mycarid, this.myForm.value, this.usuarioid).subscribe((response) => {  
                          this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                  if(response['code']==200){
                                      this.datos  = response['datos'];
  ...
  delete(num) {
    const loader = this.loadingCtrl.create({
      ...
      //message: "Un momento, por favor..."
              ...
              load.present();
              this.provider.miperfilupdate_delete(this.mycarid, this.myForm.value, this.usuarioid, num).subscribe((response) => {  
                          this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                  if(response['code']==200){
                                      this.datos              = response['datos'];
                                  }else if (response['code']==201){
