...
@Component({
  selector: 'app-perfileditarwhatsapp',
  templateUrl: 'perfileditarwhatsapp.html',
  styleUrls: ['perfileditarwhatsapp.scss'],
  providers:[Usuario, Home, Camera, File, Crop, GooglePlus, Facebook, Instagram, Geolocation]
})

export class Perfileditarwhatsapp implements  OnInit{
  ...
  constructor(public navController: NavController,
    ...
    this.myForm = this.formBuilder.group({
          ...
          codigo_pais: new FormControl('', Validators.compose([ 
                                  Validators.required
                              ])
                   ),
          ...
          codigo_valida: new FormControl('', Validators.compose([ 
                              ])
                   ),
  ...
  continuar(){
      this.siguiente = 2;
      this.codigo_pais       =  this.myForm.value.codigo_pais;
      this.telefono_whatsapp =  this.myForm.value.telefono;
      const loader = this.loadingCtrl.create({
        ////duration: 10000
        //message: "Un momento, por favor..."
      }).then(load => {
                load.present();
                this.myForm.value.codigo_pais = this.codigo_pais;
                this.myForm.value.telefono    = this.telefono_whatsapp;
                this.provider.enviarcodigowhatsapp(this.mycarid, this.myForm.value, this.usuarioid, this.image).subscribe((response) => {  
                                    ...
                                    if(response['code']==200){
                                                ...
                                                const alert = this.alertCtrl.create({ cssClass:'my-custom-class-alert',
                                                  subHeader: this.idiomapalabras.aviso,
                                                    message: this.idiomapalabras.response['msg'],
                                                    buttons: [
                                                      {
                                                        text: "Ok", 
                                                        role: 'cancel'
 ...
 salirdecargando(){
      ...
      this.mycarid   = localStorage.getItem('MYCARID');
       const loader = this.loadingCtrl.create({
          ...
          //message: "Un momento, por favor..."
                  ...
                  load.present();
                  this.provider_home.listapaises().subscribe((response) => {  
                                      if(response['code']==200){
                                                this.listapaises = response['datos'];
  ...
  actualizar_pais(var1, var2){
      ...
      this.provider_home.mapalocalizar(var1, var2).subscribe((response_aux) => {
                        /*for(var i=0; i<6 ; i++) { 
  ...
  cambiarimagen(){
    ...
    const alert = this.alertCtrl.create({ cssClass:'my-custom-class-alert',
                subHeader: this.idiomapalabras.agregarimagen,
  ...
  enviarformulario() {
    const loader = this.loadingCtrl.create({
      ...
      //message: "Un momento, por favor..."
              ...
              this.myForm.value.codigo_pais = this.codigo_pais;
              this.myForm.value.telefono    = this.telefono_whatsapp;
              this.provider.miperfilupdate(this.mycarid, this.myForm.value, this.usuarioid, this.image).subscribe((response) => {  
                          this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                  if(response['code']==200){
