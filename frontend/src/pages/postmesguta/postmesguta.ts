...
@Component({
  selector: 'app-postmesguta',
  templateUrl: 'postmesguta.html',
  styleUrls: ['postmesguta.scss'],
  providers:[SocialSharing, Usuario, Home]
})

export class Postmesguta implements  OnInit{
    @Input('usuarioid') usuarioid: string;
    @Input('mycarusuarioid') mycarusuarioid: string;
    @Input('publicacionid') publicacionid: string;
     ...
     // //this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} });
      ...
      const loader = this.loadingCtrl.create({
        ...
        ////message: "Un momento, por favor...",
                        ...
                        load.present();
                        this.provider.listarpostmegusta(this.usuarioid, this.mycarusuarioid, this.tipo, this.page_number, this.publicacionid).subscribe((response) => {  
                              this.loadingCtrl.getTop().then(loader => {if(loader!=undefined) {this.loadingCtrl.dismiss();} 
                                        this.datos  = response['usuarios'];
                                        this.datos2 = response['datos2']['data'];
